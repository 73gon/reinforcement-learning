...
class RLGui(widgets.VBox):
    ...
    def __init__(self,
        ...
        self.episode_button = widgets.Button()
        self.episode_button.on_click(lambda _: self.on_click_episode())
        self.episode_button.add_class('margin_bottom_16')
        ...
        if self.enable_user_actions:
            for action in range(env.action_space.n):
                button = widgets.Button(description=env.get_action_meaning(action), layout=dict(display='inline'))
    ...
    def render(self):
        ...
        with self.render_output:
            ...
            if self.render_mode is not None:
                ...
                render_result = self.env.render_state(state, mode=self.render_mode, value_fn=value_fn, optimal_action=self.render_optimal_action)
        ...
        with self.step_output:
            ...
            if self.print_state:
                print(f'State: {state}')
            print(f'Action: {"-" if action is None else self.env.get_action_meaning(action)}')
        ...
        if (self.terminated or self.truncated) and not waiting_for_episode:
            ...
            if self.enable_reset:
                self.episode_button.description = 'Reset'
                show_widgets.append(self.episode_button)
            else:
                hide_widgets.append(self.episode_button)
